  ...
  const stationz = () => {
    ...
    fetchData().then(dataJSON => {
      ...
      if (checkOld !== checkRecent) {
        setDose(parseFloat(JSON.parse(dataJSON)[0].Dose.toFixed(3)));

        setCPS(parseFloat(JSON.parse(dataJSON)[0].CPS));
        setTotalCounts(parseFloat(JSON.parse(dataJSON)[0].TotalCount));
        setTimeout(function () {
