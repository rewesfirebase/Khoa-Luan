...
const App = () => {
  ...
  const [cps3, setCPS3] = useState("");
  const [temp3, setTemp3] = useState("");
  const [humi3, setHumi3] = useState("");
  const [dose4, setDose4] = useState("");
  const [cps4, setCPS4] = useState("");
  const [temp4, setTemp4] = useState("");
  const [humi4, setHumi4] = useState("");
  const [dose5, setDose5] = useState("");
  const [cps5, setCPS5] = useState("");
  const [temp5, setTemp5] = useState("");
  const [humi5, setHumi5] = useState("");
  const [dose6, setDose6] = useState("");
  const [cps6, setCPS6] = useState("");
  const [temp6, setTemp6] = useState("");
  const [humi6, setHumi6] = useState("");
  const [totalCounts1, setTotalCounts1] = useState("");
  const [totalCounts2, setTotalCounts2] = useState("");
  const [totalCounts3, setTotalCounts3] = useState("");
  const [totalCounts4, setTotalCounts4] = useState("");
  const [totalCounts5, setTotalCounts5] = useState("");
  const [totalCounts6, setTotalCounts6] = useState("");
  const [stationTime1, setStationTime1] = useState("");
  const [stationTime2, setStationTime2] = useState("");
  const [stationTime3, setStationTime3] = useState("");
  const [stationTime4, setStationTime4] = useState("");
  const [stationTime5, setStationTime5] = useState("");
  const [stationTime6, setStationTime6] = useState("");
  ...
  const stationz = (number) => {
    ...
    fetchData(number).then(dataJSON => {
      ...
      if ((arrayCheckOld[number - 1] !== arrayCheckRecent[number - 1])) {
        ...
        arraySetStationTime[number - 1]("null");
        /* console.log(typeof (arrayOfArrayValue[handleClick - 1][station - 1])); */
      }
      ...
      <View style={[tailwind("flex flex-row"), styles.containCircle]}>
        ...
        <View {...touchProps}>
          <AnimatedWave
            ...
            zoom={2}
            value={arrayOfArrayValue[handleClick - 1][station - 1] !== "null" ? (typeof (arrayOfArrayValue[handleClick - 1][station - 1]) !== "string" ? (arrayOfArrayValue[handleClick - 1][station - 1] < 10 ? arrayOfArrayValue[handleClick - 1][station - 1] : nFormatter(arrayOfArrayValue[handleClick - 1][station - 1])) : arrayOfArrayValue[handleClick - 1][station - 1].substr(11, 8)) : arrayOfArrayValue[handleClick - 1][station - 1]}
